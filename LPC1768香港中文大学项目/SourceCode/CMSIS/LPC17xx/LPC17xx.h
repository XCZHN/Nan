...
typedef struct
{
  __IO uint32_t FLASHCFG;               //flash¼ÓËÙÅäÖÃ¼Ä´æÆ÷
       uint32_t RESERVED0[31];
  __IO uint32_t PLL0CON;                //PLL0¿ØÖÆ¼Ä´æÆ÷
  __IO uint32_t PLL0CFG;				//PLL0ÅäÖÃ¼Ä´æÆ÷
  __I  uint32_t PLL0STAT;			   	//PLL0×´Ì¬¼Ä´æÆ÷
  __O  uint32_t PLL0FEED;				//PLL0 Feed¼Ä´æÆ÷
       uint32_t RESERVED1[4];
  __IO uint32_t PLL1CON;				//PLL1¿ØÖÆ¼Ä´æÆ÷
  __IO uint32_t PLL1CFG;			    //PLL1ÅäÖÃ¼Ä´æÆ÷
  __I  uint32_t PLL1STAT;				//PLL1×´Ì¬¼Ä´æÆ÷
  __O  uint32_t PLL1FEED;				//PLL1 Feed¼Ä´æÆ÷
       uint32_t RESERVED2[4];
  __IO uint32_t PCON;				    //µçÔ´¿ØÖÆ¼Ä´æÆ÷
  __IO uint32_t PCONP;					//ÍâÎ§Éè±¸µçÔ´¿ØÖÆ¼Ä´æÆ÷
       uint32_t RESERVED3[15];
  __IO uint32_t CCLKCFG;				//CPUÊ±ÖÓÅäÖÃ¼Ä´æÆ÷
  __IO uint32_t USBCLKCFG;				//USBÊ±ÖÓÅäÖÃ¼Ä´æÆ÷
  __IO uint32_t CLKSRCSEL;				//Ê±ÖÓÔ´Ñ¡Ôñ¼Ä´æÆ÷
  __IO uint32_t	CANSLEEPCLR;		    //ÔÊÐíÇå³ýµ±Ç°CANÍ¨µÀË¯Ãß×´Ì¬ºÍ¶Á×´Ì¬0x400FC110
  __IO uint32_t	CANWAKEFLAGS;			//ÔÊÐí¶ÁCANÍ¨µÀµÄ»½ÐÑ×´Ì¬
       uint32_t RESERVED4[10];
  __IO uint32_t EXTINT;                 //Íâ²¿ÖÐ¶Ï±êÖ¾¼Ä´æÆ÷
       uint32_t RESERVED5;
  __IO uint32_t EXTMODE;				//Íâ²¿ÖÐ¶ÏÄ£Ê½¼Ä´æÆ÷
  __IO uint32_t EXTPOLAR;			    //Íâ²¿ÖÐ¶Ï¼«ÐÔ¼Ä´æÆ÷
       uint32_t RESERVED6[12];
  __IO uint32_t RSID;                   //¸´Î»Ô´¼ø¶¨¼Ä´æÆ÷
       uint32_t RESERVED7[7];
  __IO uint32_t SCS;                    //ÏµÍ³¿ØÖÆºÍ×´Ì¬ Syscon Miscellaneous Registers
  __IO uint32_t IRCTRIM;                /* Clock Dividers                     */
  __IO uint32_t PCLKSEL0;				//ÍâÎ§Éè±¸Ê±ÖÓÑ¡Ôñ¼Ä´æÆ÷0
  __IO uint32_t PCLKSEL1;			    //ÍâÎ§Éè±¸Ê±ÖÓÑ¡Ôñ¼Ä´æÆ÷1
       uint32_t RESERVED8[4];
  __IO uint32_t USBIntSt;               //USBÖÐ¶Ï×´Ì¬¼Ä´æÆ÷
  __IO uint32_t DMAREQSEL;			    //ÔÚÍ¨µÀ8-15ÉÏ£¬´®¿ÚºÍ¶¨Ê±DMAÇëÇóÖÐÑ¡Ôñ
  __IO uint32_t CLKOUTCFG;              //Ê±ÖÓÊä³öÅäÖÃ¼Ä´æÆ÷
 } LPC_SC_TypeDef;
