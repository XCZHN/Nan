...
{								 
    ...
    //CS初始化已在SPI初始化完成 revise by QiYuan
    /****************************************
	//SD CS初始化
	RCC->APB2ENR|=1<<3;      //PORTB时钟使能 	 
	GPIOB->CRH&=0XFFFF0FFF;  //PB11 推挽输出
	GPIOB->CRH|=0X00003000; 
	GPIOB->ODR|=1<<11;//11 输出高
	*****************************************/	 
	SPI2_Init();
	SPI2_SetSpeed(SPI2_SPEED_256);//设置到低速模式   
...
u8 SD_Read_Bytes(unsigned long address,unsigned char *buf,unsigned int offset,unsigned int bytes)
{
	...
	else r1=SD_SendCommand(CMD17,address,0);//发送读扇区命令    
