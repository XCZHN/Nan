...
#define VS_XCS_SET() GPIOB->ODR|=(1<<7)	  	//片选信号	PB7
#define VS_XCS_CLR() GPIOB->ODR&=~(1<<7)	  	//片选信号

#define VS_XRESET_SET() GPIOB->ODR|=(1<<4)  	//复位信号 PB4
#define VS_XRESET_CLR() GPIOB->ODR&=~(1<<4)  	//复位信号

#define VS_XDCS_SET()  	GPIOB->ODR|=(1<<6)		//数据片选PB6
#define VS_XDCS_CLR()  	GPIOB->ODR&=~(1<<6)		//数据片选

#define VS_DREQ    (GPIOB->IDR&(1<<5)) 			//数据请求
